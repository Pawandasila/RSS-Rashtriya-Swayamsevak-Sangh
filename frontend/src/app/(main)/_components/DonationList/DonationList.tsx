...
const DonationList = () => {
  ...
  const [filteredDonations, setFilteredDonations] = useState<DonationTickerRecord[]>([]);
  const [displayCount, setDisplayCount] = useState(10);

  useEffect(() => {
    if (searchTerm.trim()) {
      const filtered = tickerDonations.filter(donation =>
  ...
  const getPaymentIcon = (mode: string) => {
    switch (mode) {
      ...
      case 'Online': case 'Card': return <CreditCard className="w-4 h-4" />;
  ...
  const getPaymentBadgeVariant = (mode: string) => {
    switch (mode) {
      case 'UPI': return 'bg-green-50 text-green-700 border-green-200';
      case 'Online': return 'bg-blue-50 text-blue-700 border-blue-200';
      case 'Bank Transfer': return 'bg-purple-50 text-purple-700 border-purple-200';
      ...
      case 'Cheque': return 'bg-indigo-50 text-indigo-700 border-indigo-200';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            राष्ट्रीय सेवा संघ के सेवा कार्यों में योगदान देने वाले उदार हृदय दानदाताओं की सूची
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="bg-gradient-to-r from-primary/10 to-primary/5 rounded-2xl p-6 text-center">
            ...
            <h3 className="text-2xl font-bold text-foreground">{liveStats.activeDonors.toLocaleString()}</h3>
          ...
          <div className="bg-gradient-to-r from-green-50 to-green-100 rounded-2xl p-6 text-center">
            ...
            <h3 className="text-2xl font-bold text-foreground">₹{liveStats.totalAmountToday.toLocaleString()}</h3>
          ...
          <div className="bg-gradient-to-r from-orange-50 to-orange-100 rounded-2xl p-6 text-center">
            ...
            <h3 className="text-2xl font-bold text-foreground">{liveStats.totalDonationsToday}</h3>
        ...
        <div className="bg-card rounded-2xl shadow-lg border border-border overflow-hidden">
          <Table>
            <TableCaption className="py-4 text-muted-foreground">
              राष्ट्रीय सेवा संघ के दानदाताओं की सूची - कुल {tickerDonations.length} प्रविष्टियां
            </TableCaption>
            <TableHeader>
              <TableRow className="bg-muted/50">
                <TableHead className="font-semibold text-foreground px-6 py-4">दानदाता</TableHead>
                <TableHead className="font-semibold text-foreground px-6 py-4">राशि</TableHead>
                <TableHead className="font-semibold text-foreground px-6 py-4">स्थान</TableHead>
                <TableHead className="font-semibold text-foreground px-6 py-4">भुगतान माध्यम</TableHead>
                <TableHead className="font-semibold text-foreground px-6 py-4">समय</TableHead>
                <TableHead className="font-semibold text-foreground px-6 py-4">संदेश</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredDonations.map((donation) => (
                <TableRow key={donation.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-foreground">
                          {donation.isAnonymous ? 'गुप्त दानदाता' : donation.donorName}
                        </p>
                      </div>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="px-6 py-4">
                    <Badge className={`${getPaymentBadgeVariant(donation.paymentMode)} border`}>
                  ...
                  <TableCell className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-muted-foreground">{donation.timeAgo}</span>
                    </div>
                  </TableCell>
                  <TableCell className="px-6 py-4">
                    {donation.message ? (
                      ...
                      <span className="text-xs text-muted-foreground">कोई संदेश नहीं</span>
                    )}
                  </TableCell>
                </TableRow>
              ))}
        ...
        <div className="mt-16 text-center">
          <div className="bg-gradient-to-r from-primary/10 via-primary/5 to-primary/10 rounded-2xl p-8 md:p-12 max-w-4xl mx-auto">
            ...
            <p className="text-base md:text-lg text-muted-foreground leading-relaxed">
              राष्ट्रीय सेवा संघ के पवित्र कार्यों में योगदान देकर समाज सेवा का हिस्सा बनें। 
              आपका हर दान किसी के जीवन में बदलाव ला सकता है।
            </p>
          </div>
        </div>
      </div>
